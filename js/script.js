...
function setupTurnUI () {
	...
	if (gameScreen === null || document.querySelector ('[data-ui="turn-display"]') !== null) {
		return;
	}
...
function StartTypingGame () {
	...
	overlay.innerHTML = `
		<div class="typing-game-box">
			...
			<div class="typing-game-clock" id="typing-game-clock">7.0 detik</div>
		</div>
	`;
	...
	const timerFrame = function () {
		...
		clock.textContent = Math.max (0, ((TIME_LIMIT - elapsed) / 1000)).toFixed (1) + ' detik';
...
function StartDebatTimer () {
	...
	requestAnimationFrame (function () { bar.classList.add ('debat-timer-visible'); });
...
monogatari.script ({
	...
	'Event_Negosiasi_VIP': [
		...
		{
			'Choice': {
				'Dialog': 'arya Ini titik di mana aku menentukan siapa diriku sebenarnya.',
				'Terima': {
	...
	'Event_Negosiasi_Terima': [
		applyStatChanges ({ danaAnggaran: 30 }),
		{
			'Function': {
				'Apply': function () {
					adjustLeadershipStats ({ integritas: -(leadershipStats ().integritas - 15) });
					return true;
				},
				'Revert': function () {
					return true;
